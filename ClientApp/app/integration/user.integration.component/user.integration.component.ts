...
export class UserIntegrationComponent implements AfterViewInit {
    constructor(private ip: IntegrationProvider, private cd: ChangeDetectorRef) {
        ...
        this.searchTerm$.subscribe(term => {
            ...
            this.filteredUsers = this.users.filter(user => (user.firstName + ' ' + user.lastName).toLowerCase().includes(term));
            this.cd.detectChanges();
        });
    }
    ...
    private searchTerm$ = this._searchTerm.asObservable().delay(200)
        .filter(term => term != null).debounceTime(200).distinctUntilChanged();
